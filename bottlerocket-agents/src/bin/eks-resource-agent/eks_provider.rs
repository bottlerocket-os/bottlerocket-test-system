...
async fn security_group(
    ...
    // If we haven't found the security group (or we found too many), we want the user to be aware
    // of it, but we don't want the agent to exit.
    if security_groups.is_empty() {
        error!(
            "The {:?} security group was not found with filter tag:Name={}",
            security_group_type, filter_value
        );
    } else if security_groups.len() > 1 {
        error!(
            "More than one security group of type {:?} was found filter tag:Name={}: {:?}",
            security_group_type, filter_value, security_groups
        );
    }
